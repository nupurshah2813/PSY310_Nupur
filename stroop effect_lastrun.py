...
expName = 'stroop effect'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/nupurshah/Desktop/Psychopy/stroop effect_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # create an object to store info about Routine ins
    ins = data.Routine(
        name='ins',
        components=[text_2],
    )
    ins.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for ins
    ins.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    ins.tStart = globalClock.getTime(format='float')
    ins.status = STARTED
    thisExp.addData('ins.started', ins.tStart)
    ins.maxDuration = None
    # keep track of which components have finished
    insComponents = ins.components
    for thisComponent in ins.components:
    ...
    # --- Run Routine "ins" ---
    ins.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            ins.forceEnded = routineForceEnded = True
            break
    ...
    # store stop times for ins
    ins.tStop = globalClock.getTime(format='float')
    ins.tStopRefresh = tThisFlipGlobal
    thisExp.addData('ins.stopped', ins.tStop)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if ins.maxDurationReached:
        routineTimer.addTime(-ins.maxDuration)
    elif ins.forceEnded:
        routineTimer.reset()
    else:
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('/Users/nupurshah/Desktop/Psychopy/condition (lab).xlsx'), 
        seed=None, 
    )
