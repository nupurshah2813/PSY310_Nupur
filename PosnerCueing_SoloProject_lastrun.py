...
expName = 'PosnerCueing_SoloProject'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/nupurshah/Desktop/Psychopy/Solo Project/PosnerCueing_SoloProject_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions_text = visual.TextStim(win=win, name='instructions_text',
        text='Welcome to the Attention Task!\nKeep your eyes on the center cross at all times.\nA circle will appear on either the LEFT or the RIGHT side of the screen.\nPress the Z key if the circle appears on the LEFT.\nPress the M key if the circle appears on the RIGHT.\nRespond as quickly and accurately as possible.\n\nPress SPACE to begin.\n',
        font='Arial',
    ...
    cue_arrow = visual.TextStim(win=win, name='cue_arrow',
        text='',
        font='Arial',
    ...
    Instructions = data.Routine(
        name='Instructions',
        components=[instructions_text, instr_key],
    )
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('/Users/nupurshah/Desktop/Psychopy/Solo Project/PosnerCueingConditions_SoloProject.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[left_box, right_box, fixation, cue_arrow, target, response_key],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        cue_arrow.setPos((0, 0))
        cue_arrow.setText('←' if (validity=='valid' and targetSide=='left') or (validity=='invalid' and targetSide=='right') else '→'
        
        )
        target.setPos((-0.35, 0) if targetSide=='left' else (0.35, 0))
        ...
        while continueRoutine:
            ...
            if left_box.status == STARTED:
                ...
                if tThisFlipGlobal > left_box.tStartRefresh + 0.5-frameTolerance:
            ...
            if right_box.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > right_box.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # if cue_arrow is starting this frame...
            if cue_arrow.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                cue_arrow.frameNStart = frameN  # exact frame index
                cue_arrow.tStart = t  # local t and not account for scr refresh
                cue_arrow.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(cue_arrow, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'cue_arrow.started')
                # update status
                cue_arrow.status = STARTED
                cue_arrow.setAutoDraw(True)
            
            # if cue_arrow is active this frame...
            if cue_arrow.status == STARTED:
                # update params
                pass
            
            # if cue_arrow is stopping this frame...
            if cue_arrow.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > cue_arrow.tStartRefresh + cueDur_ms/1000.0 -frameTolerance:
                    # keep track of stop time/frame for later
                    cue_arrow.tStop = t  # not accounting for scr refresh
                    cue_arrow.tStopRefresh = tThisFlipGlobal  # on global time
                    cue_arrow.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'cue_arrow.stopped')
                    # update status
                    cue_arrow.status = FINISHED
                    cue_arrow.setAutoDraw(False)
            
            # *target* updates
            
            # if target is starting this frame...
            if target.status == NOT_STARTED and tThisFlip >= 0.6-frameTolerance:
                # keep track of start time/frame for later
            ...
            if response_key.status == NOT_STARTED and tThisFlip >= 0.5 + (cueDur_ms/1000.0) + (SOA_ms/1000.0) -frameTolerance:
                # keep track of start time/frame for later
            ...
            if response_key.status == STARTED and not waitOnFlip:
                theseKeys = response_key.getKeys(keyList=['z','m'], ignoreKeys=["escape"], waitRelease=False)
                ...
                if len(_response_key_allKeys):
                    ...
                    # was this correct?
                    if (response_key.keys == str('z' if targetSide=='left' else 'm' )) or (response_key.keys == 'z' if targetSide=='left' else 'm' ):
                        response_key.corr = 1
                    else:
                        response_key.corr = 0
                    # a response ends the routine
        ...
        if response_key.keys in ['', [], None]:  # No response was made
            ...
            if str('z' if targetSide=='left' else 'm' ).lower() == 'none':
